...
\title{\colorbox{green}{1-2, 3-2} Scalable Data Allocation and Parallel Computing \\ on NoC-based Embedded Many Cores}
\begin{document}
...
\colorbox{green}{TBD:1-1, 1-14, 2-2, 3-5}
...
In this paper, we use one of the embedded commercial off-the-shelf (COTS) multi/many-core components, the Multi-Purpose Processing Array (MPPA) 256 developed by Kalray, and conduct evaluations of data transfer and parallelization micro benchmark.
...
Subsequently, we run a practical application, the core of the autonomous driving system, on many-core processors and indicate acceleration by parallelization.
...
\vspace{-3mm}
\colorbox{green}{1-4, 1-5, 1-6, 2-1, 2-4}
...
\colorbox{green}{1-3} In embedded systems, Multi/many-core architecture is an important trend as it integrates cores to realize high-performance and general-purpose computing with low power consumption \cite{perret2016mapping}, \cite{faragardi2014communication}, \cite{furst2009autosar}, \cite{becker2016contention}.
...
% Recently, the computational ability of single core processors has reached its limit \colorbox{green}{1-3} and thus the applicability of Moore's law \cite{moore2006cramming} is unclear.
% Multi/many core architecture is an important trend in \colorbox{green}{1-3} recent years as it integrates cores to realize high-performance and general-purpose computing with low power consumption.
...
% The Multi-Purpose Processing Array (MPPA) 256 developed by Kalray, \cite{de2014time}, Single-chip Cloud Computer (SCC) developed by Intel \cite{baron2010single}, and Tile64 developed by Tilera \cite{bell2008tile64} include the clustering of many-core architectures in which cores are \colorbox{green}{1-7} allocated closely.
...
\colorbox{green}{1-15} % The abstraction package serves as a system that does not provide any services.
...
The following all evaluations are conducted on eMCOS.
...
\vspace{-3mm}
\subsubsection{Influence of Cache and Memory Type}
\label{sec:cache_and_memory}
First, matrix calculation time with the cache in IOS and CC is depicted in Figure \ref{fig:cat_calc_cache}.
...
When a small data cache (8 KB) in a PE of CC does not function adequately and an application always accesses data which are not in the cache memory, memory access will pay the time for a non-cached data access and the cost to refill the cache line.
...
From evaluations of D-NoC data transfer, we can get two lessons: influence of NoC routing and DMA.
First, data transfer latencies between Clusters are not hardly influenced by routing for users as shown in Figures \ref{fig:DDR_tx}, \ref{fig:DDR_uc}, \ref{fig:IO_SMEM_tx}, and \ref{fig:IO_SMEM_uc}.
...
This is profitable for user because this is common situations to transfer data from DDR to CC SMEM using UC, especially for large data.

From micro benchmark of matrix calculation, we can get three lessons of memory access: characteristic of SMEM, influence of cache, and data flow control on D-NoC.
...
It is generally known that cache overhead exists when the miss hit frequently occurs, but it is notable that there is a big difference which can not be ignored by the influence of cache line.
...
From practical application, we parallelize the vehicle self-localization algorithm of an autonomous driving system and have been shown that it can be practically used.
We apply parallel data transfer on NoC from IOS to CCs and parallel computing on IOS and CC by using CC SMEM like scratch pad memory.
...
DSPs and FPGAs are also high-performance devices when compared to CPUs.
They are efficient in terms of power consumption.
A DSP is often used for real-time systems and FPGAs guarantee predictability and efficient processing.
...
FPGAs are difficult for software developers and are not a substitute for CPU since their software model is significantly different from that of CPU.
...
MPPA-256 is typically accepted with respect to many-core platforms and the model has been used in several previous work \cite{perret2016temporal}, \cite{becker2016contention}, \cite{carle2014static}, \cite{perret2016mapping}, \cite{perret2016predictable}.

Several work have examined real-time applications on many-core platforms including MPPA-256.
...
By examining the above mapping algorithms of real-time applications, several previous work \cite{kanter2015kalray}, \cite{denet2017work}, \cite{deDinechin2014GSN}, \cite{perret2016predictable} have analyzed the potential of MPPA-256 and data transfer with NoC as shown in Table \ref{tb:comparison_relatedwork}.
...
\begin{table}[t]
  ...
  \end{tabular}
  \begin{flushright}
     \colorbox{green}{1-23} \textasteriskcentered In a table, ``L'' means ``limited''.
  \end{flushright}
  \vspace{-5mm}
\end{table}
