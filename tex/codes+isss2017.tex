...
\title{\colorbox{green}{TBD: 1-2, 3-2} Scalable data allocation and parallel computing on NoC-based embedded many cores}
\begin{document}
...
\begin{abstract}
In embedded systems, high processing requirements and reasonable power consumption needs heterogeneous computing platforms in several domains such as automobiles.
The evolution of a next generation computing platform oriented toward multi/many cores is inevitable.
Considering embedded requirements, applications need to be designed based on scalable data allocation and parallel computing with many cores.
In this paper, we investigate currently achievable data transfer between distributed memories on NoC, memory access characteristics, and parallelization potential with many cores.
Finally, we run a practical application, the core of the autonomous driving system, on many-core processors and indicate acceleration by parallelization.
...
In embedded systems, this trend is also adapted because high processing requirements are also required in several embedded systems.
For example, autonomous driving system involve various applications and are sometimes characterized by demands for high-performance computing.
Considering requirements of high processing, predictability, and significant energy efficiency for automotive systems, its system needs a heterogeneous computing system such as multi/many cores and GPUs. (This is discussed in Section \ref{sec:related_work}.)
...
A difficulty involves the sharing of numerous resources (e.g. memory subsystems and I/O devices) by many cores.
Parallelized complex processes share some resources, and this leads to the frequent occurrence of conflicts.
...
For instance, the Multi-Purpose Processing Array (MPPA) 256 developed by Kalray \cite{de2014time} adopt Non-Uniform Memory Access (NUMA) using network-on-chip (NoC).
Numerous shared resources disturb predictable timing behavior and software analysis.
...
MPPA-256 developed by Kalray \cite{de2014time} is one of the commercial off-the-shelf (COTS) multicore components targeting embedded system and adopts distributed memory architecture with NoC. (We discusses comparison to other platforms in Section \ref{sec:related_work}.)
However, its data transfer between distributed memories with NoC, parallelization potential, and memory access characteristics are not completely understood.
This study explores above issues with quantitative evaluations and a practical case study.

\textbf{Contributions:}
\colorbox{green}{1-1, 3-3: To be clarified}

This study focuses on examining embedded many-core computing based on networks-on-chip (NoCs), such as MPPA-256.
...
Finally, we parallelize and accelerate localization algorithm which becomes the core of autonomous driving system.
...
To the best of our knowledge, this is the first study that examines data transfer and data allocation matters for many-core computing beyond an intuitive expectation to allow system designers to choose appropriate data transfer methods based on the requirements of their latency-sensitive many-core applications.
